...
def doc_embedding(tokens: List[str], dim): 
    c = Counter(tokens)
    print('from explainability_global.doc_embedding, counter(tokens)= :', c, '\n')
...
def get_subspace_of_doc(txt, dim=1): # calls doc_embedding; 
 #   print("Tokenize the corpus!")
    print('type txt from get_subspace_of_doc', type(txt))
    doc_tokens = tokenizer(txt)[0] # list of tokens
    print('type doc_tokens from get_subspace_of_doc', type(doc_tokens), doc_tokens[:5])
    ...
    return U[:, :dim], words, words_emb, word_impact_no_rot, doc_tokens
...
def main(txt, label, dim, xprotos, yprotos, rel, weight_type='abs', metric_type='chordal', dic=None): # calls get_subspace_of_doc
    print('from main, type txt', type(txt))
    corpus_subspace=preprocessingfx(txt, subspace_dim=dim, model_name='word2vec-google-news-300')
   # subspace, words, words_emb, word_imp_no_rot, token = get_subspace_of_doc(txt, dim)
    plus, minus = get_winner_prototypes(corpus_subspace, label, xprotos, yprotos, rel, metric_type)
...
#fname = "../model/hyperpartisan/glove/hyperpartisan_model_d30_ps"
fname = '../model/%s/glove/%s_model_d20_ps' %(dataname, dataname)
...
if dataname == 'reuters-8':
    ...
    dic = {
        'true': 1, # hyperpartisan
        'false': 0, # not hyperpartisan
    }    
else:
    dic = None
...
main(txt, label, dim, xprotos, yprotos, rel, weight_type='nonabs', metric_type='chordal', dic=dic)
