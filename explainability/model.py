...
class Model():
    ...
    def findWinner(self, data, label):
     ...
     #   print('Data dim: ', np.shape(data), type(data))
     #   print('distances dim: ', np.shape(distances), type(distances))
    ...
    def predict(self, data):
        ...
        #else:
            #pred = 0
#        print('data & prediction distance shape:', np.shape(data), np.shape(distances))
        #NEW
        if distances.ndim == 1:
    ...
    def predict_proba(self, data):
        ...
        if data.ndim == 2:
            ...
            distances = results['distance']             
       # print('shape dist and data', np.shape(distances), np.shape(data[0]))
       # print('min dist, max dist', np.min(distances[0]), np.max(distances[0]))
       # print('Numerators and denom', distances[0], np.sum(distances[0]))
       # print('Prob pred: ',np.power(distances[0],-1)/np.sum(np.power(distances[0],-1)))
        #predict_proba = np.transpose(np.transpose(np.power(distances,-1))/np.sum(np.power(distances,-1),1))
        if distances.ndim == 1:
            predict_proba = np.exp(-distances)/sum(np.exp(-distances))
            #np.transpose(np.transpose(np.power(distances,-1))/np.sum(np.power(distances,-1),1))
        else:
            #print(np.shape(distances))
            predict_proba = np.zeros((data.shape[0],np.shape(distances)[1]))
            for i, distance in enumerate(distances):
               # print(np.sum(np.power(distance,-1)))
                predict_proba[i,:] = np.exp(-distance)/sum(np.exp(-distance))
                #np.transpose(np.transpose(np.power(distance,-1))/np.sum(np.power(distance,-1)))
        #(distances/np.sum(distances))#np.transpose(np.transpose(distances)/np.sum(distances,1))
       # print('min prob, max prob, shape prob', np.min(predict_proba), np.max(predict_proba), np.shape(predict_proba))
