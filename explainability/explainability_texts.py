...
def main(txt, label, dim, xprotos, yprotos, rel, weight_type, metric_type='geodesic', dic=None):
	...
	# CHECK: TODO: WHY  M should be word_imp_no_rot @ w['Q']
        ...
        print(txt_words_impact.shape, rel.shape, M.shape, W.shape)

        weights_sort_idx = np.argsort(txt_words_impact, axis=None)
        print(weights_sort_idx.shape)
...
def get_word_importances(txt, label, dim, xprotos, yprotos, rel, weight_type, metric_type='geodesic', dic=None):
    ...
    return word_importances_winners[0], word_importances_winners[1], words
...
# weights = main_new(txt, label, dim, xprotos, yprotos, rel, weight_type=weight_type, metric_type='pseudo-chordal', dic=dic)
pos_score, neg_score, words = get_word_importances(txt, label, dim, xprotos, yprotos, rel, weight_type=weight_type, metric_type='pseudo-chordal', dic=dic)
get_top_words(words, pos_score, neg_score, 10)
